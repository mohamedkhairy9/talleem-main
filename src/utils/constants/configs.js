...
// Roles: API returns user.roles as string array, e.g. ["entity manager", "branch manager"].
...
/**
 * Check if a menu item should be visible for the user.
 * Parent sections are shown if any child is visible (so users with only some permissions still see the section).
 * @param {Object} item - Menu item with optional allowedRoles, requiredPermission, subMenu
 * @param {string[]} normalizedUserRoles - User roles normalized
 * @param {(resource: string, action: string) => boolean} can - Permission check
 * @returns {boolean}
 */
export function isMenuItemVisible(item, normalizedUserRoles, can) {
    const roleOk = !item.allowedRoles?.length || item.allowedRoles.some(ar => normalizedUserRoles.includes(normalizeRole(ar)));
    const permissionOk = !item.requiredPermission || can(item.requiredPermission.resource, item.requiredPermission.action);

    if (item.subMenu?.length) {
        const anyChildVisible = item.subMenu.some(sub => isMenuItemVisible(sub, normalizedUserRoles, can));
...
/**
 * Filter menu tree: keep only items (and their children) that are visible.
 */
export function filterMenuByVisibility(tabs, normalizedUserRoles, can) {
    return tabs.filter(tab => isMenuItemVisible(tab, normalizedUserRoles, can)).map(tab => {
        if (!tab.subMenu?.length) return tab;
        const filteredSub = filterSubMenuByVisibility(tab.subMenu, normalizedUserRoles, can);
        return filteredSub.length ? { ...tab, subMenu: filteredSub } : { ...tab, subMenu: [] };
...
function filterSubMenuByVisibility(items, normalizedUserRoles, can) {
    ...
    for (const item of items) {
        ...
        if (item.subMenu?.length) {
            const filtered = filterSubMenuByVisibility(item.subMenu, normalizedUserRoles, can);
...
export const sideMenuTabs = [
    // نظام الحماية
    {
        ...
        icon: HiShieldCheck,
        subMenu: [
            { titleKey: 'sidebar.roles', path: '/roles', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'roles', action: 'r' } },
            { titleKey: 'sidebar.permissions', path: '/permissions', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'permissions', action: 'r' } },
            { titleKey: 'sidebar.users', path: '/users', icon: HiUser, requiredPermission: { resource: 'users', action: 'r' } },
            { titleKey: 'sidebar.activity_logs', path: '/activity-logs', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'activity_logs', action: 'r' } },
            { titleKey: 'sidebar.import_errors', path: '/import-errors', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'import_errors', action: 'r' } }
        ]
    },
        ...
        subMenu: [
            {
                ...
                subMenu: [
                    ...
                    { titleKey: 'sidebar.location_types', path: '/location-types', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'location_types', action: 'r' } },
                    { titleKey: 'sidebar.branches', path: '/branches', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'branches', action: 'r' } },
...

    {
        ...
        icon: HiUser,
        subMenu: [
            { titleKey: 'sidebar.entities', path: '/entities', icon: HiOfficeBuilding, requiredPermission: { resource: 'entities', action: 'r' } },
            { titleKey: 'sidebar.entity_managers', path: '/entity-managers', icon: HiUserGroup, requiredPermission: { resource: 'entity-managers', action: 'r' } },
            { titleKey: 'sidebar.teachers', path: '/teachers', icon: HiAcademicCap, requiredPermission: { resource: 'teachers', action: 'r' } },
            { titleKey: 'sidebar.students', path: '/students', icon: HiUserGroup, requiredPermission: { resource: 'students', action: 'r' } },
            { titleKey: 'sidebar.parents', path: '/parents', icon: HiUserGroup, requiredPermission: { resource: 'parents', action: 'r' } },
            { titleKey: 'sidebar.employees', path: '/employees', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'employees', action: 'r' } }
        ]
    },

    {
        ...
        icon: HiBell,
        subMenu: [
            { titleKey: 'sidebar.notification_templates', path: '/notification-templates', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'notifications', action: 'r' } },
            { titleKey: 'sidebar.notifications', path: '/notifications', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'notifications', action: 'r' } }
        ]
    },
        ...
        subMenu: [
            {
                ...
                icon: HiCog,
                subMenu: [
                    { titleKey: 'sidebar.request_types', path: '/request-types', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'request_types', action: 'r' } },
                    { titleKey: 'sidebar.phases', path: '/phases', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'phases', action: 'r' } },
                    { titleKey: 'sidebar.join_request_forms', path: '/join-request-forms', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'join_request_forms', action: 'r' } }
                ]
            },
            {
                ...
                icon: VscDebugBreakpointLog,
                subMenu: [
                    { titleKey: 'sidebar.join_requests_entities', path: '/join-requests/entities', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'join_requests', action: 'r' } },
                    { titleKey: 'sidebar.join_requests_teachers', path: '/join-requests/teachers', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'join_requests', action: 'r' } },
                    { titleKey: 'sidebar.join_requests_supervisors', path: '/join-requests/supervisors', icon: VscDebugBreakpointLog, requiredPermission: { resource: 'join_requests', action: 'r' } }
                ]
            }
        ]
    },
